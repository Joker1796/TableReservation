...
<template>
    <Card class="w-full">
        <CardHeader>
            <CardTitle class="flex gap-3"> <LockKeyhole class="size-4" />Коды восстановления 2FA </CardTitle>
            <CardDescription>
                Коды восстановления позволяют восстановить доступ, если вы потеряете устройство 2FA. Храните их в
        ...
        <CardContent>
            <div class="flex flex-col gap-3 select-none sm:flex-row sm:items-center sm:justify-between">
                <Button @click="toggleRecoveryCodesVisibility" class="w-fit">
                    ...
                    {{ isRecoveryCodesVisible ? 'Скрыть' : 'Показать' }} коды восстановления
                </Button>
            ...
            >
                ...
                <div v-else class="mt-3 space-y-3">
                    ...
                    <p class="text-xs text-muted-foreground select-none">
                        Каждый код можно использовать один раз — после использования он будет удалён. Если нужны новые,
                        нажмите
                        <span class="font-bold">«Пересоздать коды»</span> выше.
                    </p>
                </div>
            </div>
        </CardContent>
